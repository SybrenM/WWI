...
<p>Klik op "Bestelling afbreken" om de bestelling af te breken.</p>
        <div class="row">
          <div class="col-50">
            ...
            <input type="text" id="fname" name="Voornaam" placeholder="Henk de Groot" value="<?php if(isset($_POST["Voornaam"])){echo ($_POST["Voornaam"]);} ?>">
            <label for="email"><i class="fa fa-envelope"></i> Email</label>
            <input type="text" id="email" name="email" placeholder="hdegroot@example.com" value="<?php if(isset($_POST["email"])){echo ($_POST["email"]);} ?>">
            <label for="adr"><i class="fa fa-address-card-o"></i> Adres</label>
            <input type="text" id="adr" name="address" placeholder="Dorpstraat 14" value="<?php if(isset($_POST["address"])){echo ($_POST["address"]);} ?>">
            <label for="city"><i class="fa fa-institution"></i> Plaats</label>
            <input type="text" id="city" name="city" placeholder="Zwolle" value="<?php if(isset($_POST["city"])){echo ($_POST["city"]);} ?>">
            <label for="land"><i class="fa"></i> Land</label>
            <input type="text" id="land" name="land" placeholder="Nederland" value="<?php if(isset($_POST["land"])){echo ($_POST["land"]);} ?>">

            <div class="row">
              <div class="col-50">
                <label for="zip">Postcode</label>   
                <input type="text" id="zip" name="zip" placeholder="9999XY" value="<?php if(isset($_POST["zip"])){echo ($_POST["zip"]);} ?>">
                            <label for="telefoonnummer"><i class="fa"></i> Telefoonnummer</label>
            <input type="text" id="telefoonnummer" name="telefoonnummer" placeholder="0623478282" value="<?php if(isset($_POST["telefoonnummer"])){echo ($_POST["telefoonnummer"]);} ?>">
          ...
          <div class="col-50">
            ...
            <input type="text" id="cname" name="cardname" placeholder="Henk de Groot" value="<?php if(isset($_POST["cardname"])){echo ($_POST["cardname"]);} ?>">
            <label for="ccnum">IBAN</label>
            <input type="text" id="ccnum" name="IBAN" placeholder="NL70 RABO 0123 4567 89" value="<?php if(isset($_POST["IBAN"])){echo ($_POST["IBAN"]);} ?>">
            <div class="row">
              <div class="col-50">
                <label for="pasnummer">Pasnummer</label>
                <input type="text" id="pasnummer" name="pasnummer" placeholder="3522" value="<?php if(isset($_POST["pasnummer"])){echo ($_POST["pasnummer"]);} ?>">
              </div>
            </div>
          </div>
